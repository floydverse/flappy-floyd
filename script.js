...
// If you still have Firebase, keep it. Otherwise remove or comment out:
// var firebaseConfig = {...}
...
// OFFLINE user fent => stored in localStorage
...
const gameOverImage=document.getElementById("gameOverImage"); // now pixelated2.png
...
function startGame(){
  startBtn.style.display="none"; // hide once we start
  gameState="running";
  resetGame();
  try{
...
function gameOver(){
  ...
  if(score> highScore){
    highScore= score;
    localStorage.setItem("flappyFloydHighScore", highScore);
  }
  updateScoreUI();
}
function spawnPipe(){
  ...
  if(gameState==="running"){
    ...
    for(let i=0;i<pipes.length;i++){
      ...
      if(!p.markedForDespawn){
        ...
        const bottomRect={ x:p.x,y:p.y+offset,width:pipeWidth,height:pipeHeight };
    ...
    for(let b=0;b<bottles.length;b++){
      ...
      if(isColliding(birdRect, fentRect)){
        ...
        // If you still have Firebase or similar, you'd store fent data there. 
        // Otherwise, remove or comment out.
...
// RESTART => re-play music so we have soundtrack again
...
// Local Leaderboard + name-based user fent
let leaderboard=[];
...
// If you have no database, store in local array
...
function shareScore(){
  ...
  if(!rawName)return;
  let sharedScore= highScore;

  // push
  leaderboard.push({name: rawName, score: sharedScore});
  leaderboard.sort((a,b)=> b.score-a.score);
  leaderboard= leaderboard.slice(0,10);
...
// offline user fent label
userFentTotal.textContent= offlineUserFent.toString();

// global
let storedGlobal= localStorage.getItem("globalFentCollected");
if(storedGlobal){
  globalFentTotal.textContent= storedGlobal;
} else {
  globalFentTotal.textContent="0";
}
